...
coords = [1:s for s ∈ size(AN.Plots.rotatereferenceatlas(vol))./2]
coords[3] = .-coords[3]
...
f = volume!(ax, coords..., AN.Plots.rotatereferenceatlas(vol)[1:2:end, 1:2:end, 1:2:end]; algorithm=:mip, colorrange=extrema(vol), colormap=collect(grad), ticks=nothing, fxaa=true, transparency=true)
