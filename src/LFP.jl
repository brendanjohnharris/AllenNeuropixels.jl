...
function stimuluspartition(session, probeids, structures, stim; inbrain = 0.0,
    ...
    if structures isa AbstractString
...
function spontaneouspartition(session, probeids, structures, duration; inbrain = 0.0)
    ...
    for p in eachindex(probeids)
        ...
        _X = []
        t = minimum(dims(X, 𝑡))
        while t + duration < maximum(dims(X, 𝑡))
            push!(_X, X[𝑡(t .. (t + duration))])
            t += duration
        end
        push!(Y, _X)
    end
...
function thetafeature(Y::Vector{<:AbstractVector}; kwargs...) # Input formatted as stimuluspartition(
    t = [[([mean(dims(c, 𝑡)) for c in eachcol(s)]) for s in y] for y in Y]
...
function fooof(p::LogWaveletMatrix, freqrange = [1.0, 300.0]; max_n_peaks = 10,
               aperiodic_mode = "knee", peak_threshold = 0.5, kwargs...)
    ffreqs = 10.0 .^ collect(dims(p, Log𝑓))
...
function logaperiodicfit(p::LogWaveletMatrix, freqrange = [1.0, 300.0], args...;
                         doplot = false, mink = 0.01, kwargs...)
    fm = fooof(p, freqrange, args...; kwargs...)
    if doplot != false
    ...
    if aperiodic_mode == "fixed"
        b, χ = pyconvert.((Float64,), fm.aperiodic_params_)
        k = 0.0
    else
...
aperiodicfit(args...; kwargs...) = f -> (logaperiodicfit(args...; kwargs...)(log10(f)))

function _fooofedwavelet(res::LogWaveletMatrix, args...; kwargs...)
    psd = mean(res, dims = 𝑡)
    ffreqs = dims(psd, Log𝑓) |> collect
